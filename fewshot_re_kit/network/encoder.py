...
class Encoder(nn.Module):
    ...
    def pcnn(self, inputs):
        ...
        #x_cat = F.relu(x_cat)
        #x_pool = self.pool(x)   #400 230 1
        #bert = F.relu(self.fc(bert))
        x_ori = self.bpool(x)
        
        #x_cat = torch.cat([x_pool,bert_pool],1)
        #x_cat = self.fc2(x_cat.transpose(1,2))
        #x_cat = F.tanh(x_cat.transpose(1,2))
        ...
        return x_ori#,x_nopool#,finalatt
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, inp, mask = None,dmask = None):
        
        rval = []
        #ipdb.set_trace()
        pre_output = inp #(400,40,75)
        #ipdb.set_trace()
        mask = mask.float()
        masks = torch.matmul(mask[:, :, None], mask[:, None, :]).float() #(400,40,40)
        #pre_output *= mask.unsqueeze(2)
        for i in range(self.num_layers):
            #pre_output *= mask.unsqueeze(2)
            cur_output = self.transformer[i](pre_output, masks)
            rval.append(cur_output)
            pre_output = cur_output  #(400,40,75)
        #pre_output = pre_output.masked_fill(1 - mask.unsqueeze(2).byte(),  -1e18)
        #
        pre_output = self.mlp_final(pre_output)
        x_for_att = pre_output 
        #pre_output = pre_output*dmask.unsqueeze(-1)
        x = self.pool(pre_output.transpose(1, 2))		
        x_mask = x.transpose(1,2)
        #ipdb.set_trace()
        #=================================================================
        #x_for_att = self.fc(x_for_att)       
        #x_mask = self.fc(x_mask)
        #w_att_score = torch.tanh(x_for_att*x_mask).sum(-1)
        #att_score = F.softmax(w_att_score ,dim=-1)
        #loss2 = self.cost(w_att_score,mask)
        #att_x = (x_for_att * att_score.unsqueeze(-1)).sum(-2).unsqueeze(-1)
        #=================================================================
        return x_for_att
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       # print inp.size()
		#f3=open('multi_head_op.txt','a')  
		#print >>f3, context
		#f3.close()
		#print context.size() #(1,16,48)
